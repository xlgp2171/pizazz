...
class Hnc2String {
     ...
     * max : 0xFFFF
     */
    static final String[] MAP = new String[65536];

    static {
        Pattern p = Pattern.compile("0x([0-9a-f]{4})\\s*=>\\s*\\[" +
				"0x([0-9a-f]{4})" +
				"(?:,\\s*0x([0-9a-f]{4}))?" +
				"(?:,\\s*0x([0-9a-f]{4}))?" +
				"\\]", Pattern.CASE_INSENSITIVE);

        try (InputStream resource = Hnc2String.class.getClassLoader().getResourceAsStream("hnc2unicode.rb")) {
            int lineNumber = 0;
